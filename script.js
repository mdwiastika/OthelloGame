...
function draw_pawn_step() {
  for (let iter_step = 0; iter_step < arr_pawn_step.length; iter_step++) {
    square = document.getElementById(
      `square${arr_pawn_step[iter_step].x}${arr_pawn_step[iter_step].y}`
    );
...
function flip_pawn(affected_pawn) {
  for (let i = 0; i < affected_pawn.length; i++) {
    ...
    color_othello_board[pawn_position.y][pawn_position.x] == "pawn-black"
      ? (color_othello_board[pawn_position.y][pawn_position.x] = "pawn-white")
      : (color_othello_board[pawn_position.y][pawn_position.x] = "pawn-black");
  }
}
